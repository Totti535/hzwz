...
import com.hzwz.tailbase.common.Caller;
import com.hzwz.tailbase.common.Resp;
import com.hzwz.tailbase.utils.BaseUtils;
...
public class MessageHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    private static final List<TraceIdBucket> TRACEID_BUCKET_LIST = new ArrayList<>(Constants.BACKEND_BUCKET_COUNT);
    ...
    public static void init() {
        for (int i = 0; i < Constants.BACKEND_BUCKET_COUNT; i++) {
    ...
    private void consumeTraceDetails(Map<String, List<String>> detailMap, int pos) {
        ...
        int p = pos % Constants.BACKEND_BUCKET_COUNT;
    ...
    public void setWrongTraceId(Set<String> badTraceIdSet, int pos) {
        int bucketPos = pos % Constants.BACKEND_BUCKET_COUNT;
        ...
        if (processCount >= Constants.TARGET_PROCESS_COUNT) {
    ...
    public static boolean isFin() {
        // 是否收到对应次FIN信号
        if (FIN_TIME.get() < Constants.TARGET_PROCESS_COUNT) return false;
        // bucket中元素是否消费完
