package com.hzwz.tailbase.client;

import com.alibaba.fastjson.JSON;
import com.hzwz.tailbase.BasicHttpHandler;
import com.hzwz.tailbase.Constants;
import com.hzwz.tailbase.common.Resp;
import com.hzwz.tailbase.utils.WsClient;
...
public class ClientDataStreamHandler implements Runnable {
    ...
    public static void init() {
        for (int i = 0; i < Constants.CLIENT_BIG_BUCKET_COUNT; i++) {
            ...
            bigBucket.init();
            DataStorage.BUCKET_TRACE_LIST.add(bigBucket);
            DataStorage.DATA_LIST.add(new ArrayList<>(10));
        }
    }

    public static void start() {
        DataStorage.START_POOL.execute(new ClientDataStreamHandler());
    }
    ...
    private static void updateWrongTraceId() {
        ...
        DataStorage.updateDataQueue.offer(msgBuilder.toString());
        msgBuilder.delete(0, msgBuilder.length());

        if (DataStorage.updateDataQueue.size() >= 10 || isFin) {
            updateMsgBuilder.append("{\"type\":").append(Constants.UPDATE_TYPE).append(", \"data\": [");
            while (DataStorage.updateDataQueue.size() > 0) {
                String tmp = DataStorage.updateDataQueue.poll();
    ...
    private static void getWrongTraceWithBucketPos(int bucketPos, int pos, Set<String> errTraceIdSet
        ...
        BigBucket bigBucket = DataStorage.BUCKET_TRACE_LIST.get(bucketPos);
        List<TraceIndexBucket> traceIndexBucketList = bigBucket.getSmallBucketList();
        List<byte[]> tmpDataBucket = DataStorage.DATA_LIST.get(bucketPos);
        List<String> spans;
        int bucketCount = Constants.CLIENT_BIG_BUCKET_COUNT;
        int tmpBlockOffset;
        int tmpStartPos;
                        ...
                        for (int[] spansIndex : spansIndexes) {
                            ...
                            if (tmpStartPos <= tmpEndPos) {
                                ...
                                if (tmpDataBucket.size() > tmpBlockOffset + 1) {
                                    ...
                                    // 不是一个bucket中的
                                    List<byte[]> bytesList = DataStorage.DATA_LIST.get((bucketPos + 1) % Constants.CLIENT_BIG_BUCKET_COUNT);
        ...
        if (shouldClear) {
            DataStorage.DATA_LIST.get(bucketPos).clear();
        }
    }

    private void callFinish() {
        websocket.sendTextFrame(Constants.finMsg);
        // *info("已发送 FIN 请求");
    }
    ...
    public static class BlockWorker {
                ...
                if (bt == lf) {
                    ...
                    if (!isFirstLine) {
                        ...
                        isWrongSpan = !fastContains(bytes,
                                tagsStartPos, i, Constants.standardBytes[2]) && (fastContains(bytes,
                                tagsStartPos, i, Constants.standardBytes[1]) || fastContains(bytes, tagsStartPos, i, Constants.standardBytes[0]));
        ...
        public static void handleLine(String line, StringBuilder traceIdBuilder, StringBuilder tagsBuilder, int firstPartLineEndPos) {
            ...
            if (dataBucket.size() - 2 < 0) {
                ...
                prePos = DataStorage.DATA_LIST.get(preBucketPos).size() - 1;
                if (prePos < 0) {
